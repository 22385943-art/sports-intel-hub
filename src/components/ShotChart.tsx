...
// 🚀 1. GEOMETRÍA SVG EXACTA (TESELACIÓN MATEMÁTICA)
// El radio de 238.66 garantiza que todos los arcos compartan el centro exacto (0,0), creando un círculo perfecto.
const MACRO_ZONES = [
  ...
  { id: "mid_left_wing", path: "M -220 92.5 A 238.66 238.66 0 0 1 -80 224.85 L -80 92.5 Z", cx: -140, cy: 140, expectedPct: 0.41, name: "Left Wing Mid" },
  { id: "mid_right_wing", path: "M 80 224.85 A 238.66 238.66 0 0 1 220 92.5 L 80 92.5 Z", cx: 140, cy: 140, expectedPct: 0.41, name: "Right Wing Mid" },
  { id: "mid_center", path: "M -80 142.5 L -80 224.85 A 238.66 238.66 0 0 1 80 224.85 L 80 142.5 Z", cx: 0, cy: 180, expectedPct: 0.42, name: "Center Mid" },
  ...
  { id: "atb3_left", path: "M -250 92.5 L -220 92.5 A 238.66 238.66 0 0 1 -80 224.85 L -80 320 L -250 320 Z", cx: -165, cy: 260, expectedPct: 0.35, name: "Above Break 3 Left" },
  { id: "atb3_center", path: "M -80 224.85 A 238.66 238.66 0 0 1 80 224.85 L 80 320 L -80 320 Z", cx: 0, cy: 270, expectedPct: 0.35, name: "Above Break 3 Center" },
  { id: "atb3_right", path: "M 80 224.85 A 238.66 238.66 0 0 1 220 92.5 L 250 92.5 L 250 320 L 80 320 Z", cx: 165, cy: 260, expectedPct: 0.35, name: "Above Break 3 Right" },
];
...
export default function ShotChart({ shots }: ShotChartProps) {
  ...
  // 🚀 2. CLASIFICADOR TRIGONOMÉTRICO SINCRONIZADO CON LA GEOMETRÍA
  const zoneStats = useMemo(() => {
    ...
    shots.forEach(shot => {
      ...
      // Lógica matemática idéntica a las fronteras del SVG
      let is3pt = false;
      if (y <= 92.5) {
  ...
  // 🚀 3. ESCALA DE COLORES (Verde = Éxito, Rojo = Fallo, Gris = Promedio)
  const getRelativeHeatColor = (made: number, total: number, expected: number) => {
    const pct = made / total;
    const diff = pct - expected;
    
    if (diff >= 0.08) return "#10b981"; // Verde Brillante (Élite)
    if (diff >= 0.02) return "#34d399"; // Verde Suave
    if (diff >= -0.02) return "#64748b"; // Gris Pizarra (Media de la liga)
    if (diff >= -0.08) return "#f87171"; // Rojo Suave
    return "#ef4444"; // Rojo Intenso (Peligro)
  };
          ...
          <g transform="scale(-1, 1)"> 
            <g transform="scale(1, -1)"> 
              ...
              {viewMode === "zones" && zoneStats.map((bin) => {
                ...
                // Control del Volumen: Zonas con poca muestra se funden sutilmente con el fondo
                ...
                return (
                  <path
                    ...
                    stroke="rgba(255,255,255,0.05)" // Línea divisoria muy sutil para encajar las piezas
                    strokeWidth={1}
              ...
              {/* 🚀 CAPA 2: ESQUELETO DE LA PISTA (DIBUJADO ENCIMA DEL COLOR Y MÁS BRILLANTE) */}
              <g className="pointer-events-none" stroke="rgba(255,255,255,0.4)" strokeWidth="2.5" fill="none">
                <path d="M -220 -47.5 L -220 92.5 A 238.66 238.66 0 0 1 220 92.5 L 220 -47.5" strokeWidth="3" stroke="rgba(255,255,255,0.6)" />
              ...
              {/* CAPA 3: PUNTOS SCATTER (SÓLO EN MODO SCATTER) */}
              ...
              {/* CAPA 4: TEXTO ESTADÍSTICO SIEMPRE LEGIBLE */}
              {viewMode === "zones" && zoneStats.map((bin) => {
                const pct = (bin.made / bin.total) * 100;
                return (
