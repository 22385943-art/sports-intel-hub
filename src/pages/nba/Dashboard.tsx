...
import { NBA_PLAYERS, NBA_TEAMS, computeGIR, computeTeamMetrics } from "@/data/nba/mockData";
...
export default function NBADashboard() {
  ...
  const avgPPG = (NBA_PLAYERS.reduce((s, p) => s + p.stats.ppg, 0) / NBA_PLAYERS.length).toFixed(1);
  const topGIR = [...NBA_PLAYERS].sort((a, b) => computeGIR(b) - computeGIR(a))[0];
  const bestTeam = topTeams[0];
  const bestTeamMetrics = computeTeamMetrics(bestTeam);

  const metricTiles = [
    { title: "Players Tracked", value: NBA_PLAYERS.length, icon: Users, sparkData: [8, 9, 10, 10, 10, 10], color: "hsl(var(--chart-teal))" },
    { title: "Teams", value: NBA_TEAMS.length, icon: Shield, sparkData: [9, 9, 9, 9, 9, 9], color: "hsl(var(--chart-blue))" },
    { title: "Avg PPG", value: avgPPG, icon: TrendingUp, sparkData: [25.1, 25.8, 26.2, 26.9, 27.1, 27.3], color: "hsl(var(--chart-gold))" },
    { title: "Top GIR", value: computeGIR(topGIR), icon: Activity, sparkData: [42, 44, 45, 47, 48, 50], color: "hsl(var(--chart-teal))" },
