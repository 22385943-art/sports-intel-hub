...
Versión DEFINITIVA v3: Puerta de Integridad + PROTOCOLO LÁZARO (Solver Deductivo).
"""
...
# ─── MOTOR DE ESTADO (RELOJ BIOMÉTRICO) ───
class StateMachine:
    ...
    def deduce_period_starters(self, period_events: pd.DataFrame) -> None:
        ...
        for _, row in period_events.iterrows():
            ...
            p1_id, p1_team = _clean_id(row['player1_id']), _clean_id(row['player1_team_id'])
            p2_id, p2_team = _clean_id(row['player2_id']), _clean_id(row['player2_team_id'])
            p3_id, p3_team = _clean_id(row['player3_id']), _clean_id(row['player3_team_id'])
...
def process_game(game_id: str, game_date: Any, home_team: str, away_team: str, pbp_df: pd.DataFrame, stats_df: pd.DataFrame) -> Optional[pd.DataFrame]:
    rows = []
    ...
    state = StateMachine(home_team, away_team)
    ...
    for period, period_df in pbp_df.groupby('period'):
        ...
        for _, row in period_df.iterrows():
            ...
            p1_id, team1_id = _clean_id(row['player1_id']), _clean_id(row['player1_team_id'])
            ...
            if off_team is None and team1_id and team1_id != "0":
            ...
            if evt_type == EVT_MADE_SHOT:
                pts = 3 if "3PT" in str(row['homedescription'] or row['visitordescription'] or "").upper() else 2
                ...
                desc = str(row['homedescription'] or row['visitordescription'] or "UNKNOWN").upper()
                _emit_possession(PossessionResultType.TURNOVER.value, turnover_type=desc.split()[0] if desc else "UNKNOWN")
                off_team, possession_start_clock, fts_awarded, fts_made = None, None, 0, 0
                action_bucket += 1
            elif evt_type == EVT_FOUL:
                is_shoot = "S.FOUL" in str(row['homedescription'] or row['visitordescription'] or "").upper()
                ...
                fts_awarded += 1
                if "MISS" not in str(row['homedescription'] or row['visitordescription'] or "").upper(): fts_made += 1
...
# ==============================================================
    # 🧬 PROTOCOLO LÁZARO: Curación por Biyectividad Estricta 🧬
    # ==============================================================
    # 1. Extraer la verdad oficial (Box Score)
    ...
    # 2. Agrupar fantasmas detectados por equipo
    ...
    # 3. Identificar TODOS los candidatos por DÉFICIT de minutos (La cura para la Fuga Parcial)
    deficits_home = {}
    deficits_away = {}
    ...
    for _, stats_row in stats_df.iterrows():
        ...
        if pid and off_mins > 0:
            ...
            # Si a un jugador le faltan más de 2 minutos (déficit), es sospechoso de ser un fantasma parcial
            if deficit > 2.0:
                if pteam == home_team: deficits_home[pid] = deficit
                elif pteam == away_team: deficits_away[pid] = deficit

    # 4. Resolver asignaciones con control estricto de AMBIGÜEDAD
    ghost_mapping = {}
    ...
    for team_name, team_ghosts, team_deficits in [("HOME", ghosts_home, deficits_home), ("AWAY", ghosts_away, deficits_away)]:
        ...
        # Para cada fantasma, buscamos cuántos candidatos (déficits) encajan en sus minutos
        safe_to_map = True
        for g_id, g_mins in team_ghosts.items():
            candidatos_validos = []
            
            for pid, def_mins in team_deficits.items():
                # Si el déficit del jugador se parece a los minutos del fantasma (Tolerancia 3.5m)
                if abs(g_mins - def_mins) <= 3.5:
                    candidatos_validos.append(pid)
            
            # Si hay EXACTAMENTE UN candidato que encaja, lo emparejamos
            if len(candidatos_validos) == 1:
                ghost_mapping[g_id] = candidatos_validos[0]
                # Retiramos al candidato para que no sea usado por otro fantasma
                del team_deficits[candidatos_validos[0]] 
                
            # 🚨 SI HAY MÁS DE UN CANDIDATO, HAY AMBIGÜEDAD (El caso LeBron vs Bronny) 🚨
            elif len(candidatos_validos) > 1:
                logger.warning(f"⚠️ [AMBIGÜEDAD] Fantasma de {g_mins:.1f}m tiene múltiples candidatos viables: {candidatos_validos}. Curación abortada.")
                safe_to_map = False
                break
                
        if not safe_to_map:
            ghost_mapping.clear() # Abortamos toda la curación de este equipo
            break

    # 5. Aplicar la cura solo si hemos resuelto TODOS los fantasmas del partido de forma segura
    ...
    if total_ghosts_detected > 0 and len(ghost_mapping) == total_ghosts_detected:
        logger.info(f"🧬 [LÁZARO] Salvando partido {game_id}: Reparando quirúrgicamente {len(ghost_mapping)} jugador(es) fantasma(s) SIN AMBIGÜEDAD.")
        for row in rows:
            for i in range(1, 6):
        ...
        # Sincronizar el reloj de minutos inferidos para pasar la puerta de integridad
        for g_id, l_id in ghost_mapping.items():
            state.inferred_seconds[l_id] = state.inferred_seconds.pop(g_id)

    # ==============================================================
    # 🚨 PUERTA DE INTEGRIDAD FINAL 🚨
    # ==============================================================
    corrupted = False
    for pid, official_minutes in real_players_in_stats.items():
        inferred_minutes = state.inferred_seconds.get(pid, 0.0) / 60.0
        
        # Tolerancia post-curación
        if abs(official_minutes - inferred_minutes) > 3.5:
            logger.warning(f"❌ [CORRUPCIÓN] Jugador {pid} - Oficial: {official_minutes:.1f}m | Inferido: {inferred_minutes:.1f}m. Partido descartado.")
            corrupted = True
            break
            
    if corrupted: return None
        
    return pd.DataFrame(rows, columns=list(EXPECTED_POSSESSION_COLUMNS))
...
def main():
    ...
    for p in [KAGGLE_GAME_PATH, KAGGLE_PBP_PATH, KAGGLE_STATS_PATH]:
        if not p.exists():
            return logger.error(f"Falta archivo requerido: {p}")
    ...
    logger.info(f"Procesando {len(game_ids)} partido(s) de la era moderna desde CSV local...")
    ...
    if not chunks: return logger.error("No se encontraron eventos en el PBP.")
    ...
    for _, game_row in target_games.iterrows():
        ...
        g_date = game_row['game_date']
        h_team, a_team = _clean_id(game_row['team_id_home']), _clean_id(game_row['team_id_away'])
        
        game_events = pbp_df[pbp_df['game_id'] == g_id].sort_values('eventnum')
        game_stats = stats_df_full[stats_df_full['gameId'] == g_id]
        
        if game_events.empty or game_stats.empty: 
            logger.warning(f"Omitiendo {g_id}: faltan datos PBP o Stats oficiales.")
            continue
            
        logger.info(f"Auditando partido {g_id}...")
        pos_df = process_game(g_id, g_date, h_team, a_team, game_events, game_stats)
        
        if pos_df is not None:
            frames.append(pos_df)
            logger.info(f"✅ [OK] Partido {g_id} verificado y superado. Posesiones: {len(pos_df)}")
        else:
            rechazados += 1

    logger.info(f"RESULTADO FINAL: {len(frames)} válidos | {rechazados} rechazados por corrupción.")
    
    if frames:
        final_df = pd.concat(frames, ignore_index=True)
        out_path = Path("data/historical/kaggle_possessions_test.parquet")
        final_df.to_parquet(out_path, index=False)
        logger.info(f"🏆 ¡ÉXITO! {len(final_df)} posesiones limpias guardadas en {out_path}.")
    else:
        logger.error("¡ALERTA! Todos los partidos de prueba fallaron la Puerta de Integridad.")
